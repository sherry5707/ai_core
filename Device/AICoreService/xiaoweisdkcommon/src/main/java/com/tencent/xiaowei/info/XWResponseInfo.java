...
public class XWResponseInfo implements Parcelable {
    private static final String TAG = "XWResponseInfo";
    ...
    public static void dumpXWResponseInfo(String tag, XWResponseInfo info) {
        Log.d(tag, "dumpXWResponseInfo() called with: info = [" + info.toString2() + "]");
        for (XWResGroupInfo resGroupInfo : info.resources) {
            for (XWResourceInfo xwResInfo : resGroupInfo.resources) {
                Log.d(tag, "dumpXWResponseInfo: " + xwResInfo);
            }
        }
    }
}