...
public class XWSDK {
    ...
    public String request(int type, byte[] requestData, XWContextInfo context) {
        ...
        if (TextUtils.isEmpty(strVoiceID)) {
     ...
     * @return 本次请求对应的voiceID
     */
    public String requestWithCallBack(int type, byte[] requestData, XWContextInfo context, RequestListener listener) {
        String strVoiceID = XWSDKJNI.request(type, requestData, context);
    ...
    public int requestCancel(String voiceId) {
        if(online == true) {
            //offline or unbind status the function will block here.
     ...
     */
    public String setFavorite(XWAppInfo appInfo, String playId, boolean isFavorite) {
        return XWSDKJNI.setFavorite(appInfo, playId, isFavorite);
    }
