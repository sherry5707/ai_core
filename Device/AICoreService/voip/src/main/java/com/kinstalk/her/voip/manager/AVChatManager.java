...
import com.kinstalk.her.voip.activity.manager.TXRecordService;
import com.kinstalk.her.voip.recevier.MissCallReceiver;
...
public class AVChatManager {
    ...
    public static final String ACTION_MISS_CALL="com.kinstalk.her.voip.ACTION_MISS_CALL";
    private static final String mAVPackageName = "com.kinstalk.her.voipcall";
    public static  final String ACTION_ENTER_VOIP = "ACTION_ENTER_VOIP_CALL";
    public static  final String ACTION_QUIT_VOIP = "ACTION_QUIT_VOIP_CALL";
    ...
    private boolean mIsInvited;//是否被邀请通话
    ...
    private TXRecordService recordService;//通话记录管理

    private long selfDin = XWDeviceBaseManager.getSelfDin();
    ...
    private ServiceConnection mConn = new ServiceConnection() {
        ...
        public void onServiceDisconnected(ComponentName name) {
            ...
            privacyManager.destory();
            QLog.d(TAG,"  recordService.destory();");
            if(null!=skillReceiver){
                mContext.unregisterReceiver(skillReceiver);
            }
        }
    };
    ...
    public void init(Context context) {
        ...
        MissCallReceiver.registerReceiver(context);
        this.recordService = new TXRecordService(context.getApplicationContext());
        audioFocusManager = AudioFocusManager.getInstance(context);
        mAudioManager = (AudioManager) mContext.getSystemService(Context.AUDIO_SERVICE);
        privacyManager = PrivacyManager.getInstance(context);
        QLog.d(TAG,"  new recordService");
        //注册skill广播
        IntentFilter filter = new IntentFilter();
        filter.addAction(ACTION_MAKE_VIDOE_CALL);
        context.registerReceiver(skillReceiver,filter);
...
//        recordService.init(mIsInvited, String.valueOf(mCurrentPeerId), String.valueOf(selfDin));
//        recordService.initCreate();
        QLog.d(TAG," recordService.initCreate()");
    }
                      ...
                      /*      play(playList, new MusicPlayer.OnPlayListener() {
                                ...
                                public void onCompletion(int error) {
                                    ...
                                    if (service != null && !TextUtils.isEmpty(responseInfo.context.ID) && mState > 0) {
                                        service.wakeup(responseInfo.context.ID, 5000, 500);
                                    }
                                }
                            });*/
  ...
  /*  private void play(final ArrayList<XWResourceInfo> playList, final MusicPlayer.OnPlayListener listener) {
    ...
    public static int checkCommandId(XWResGroupInfo[] resources) {
    ...
    public static String getCommandValue(XWResGroupInfo[] resources) {
        if (resources != null && resources.length > 0 && resources[0].resources.length > 0 && resources[0].resources[0].format == XWCommonDef.ResourceFormat.COMMAND) {
    ...
    public void setVideoPID(int pid, String videoService) {
        ...
        if (pid == 0) {
            ...
            if (mIXWAudioChatService == null) {
                ...
                Intent intent = new Intent();
                intent.setAction("com.kinstalk.her.voipcall.RemoteTXAudioChatService");
    ...
    public void setCalling(boolean in) {
        if (in) {
            setChatState(3);
            recordService.onVideoConnected();
            QLog.d(TAG," recordService.onVideoConnected();");
            // todo 开始给语音
        } else {
            recordService.onVideoEnd();
            QLog.d(TAG," recordService.onVideoEnd();");
            // todo 结束给语音
        }
    }
...
//            }
    ...
    public void startAudioVideoChat(long peerId) {
        QLog.d(TAG, "start AudioVideoChat " +peerId +"mIsAudioMode"+mIsAudioMode);
        if (peerId == 0) {
            return;
        }
    ...
    public void setChatState(int state) {
        QLog.d(TAG, "setChatState" +state + " mPreState" +mPrevState);
        if (mState != state) {
            mPrevState = mState;
            mState = state;
            ...
            //发广播通知AICore 当前通话状态,保证enter和quit的广播是成对发送
            if (mState == 0 && isConnected){
                QLog.i(TAG,"send voip quit broadcast,action = ACTION_QUIT_VOIP_CALL");
                Intent intent = new Intent();
                intent.setAction(ACTION_QUIT_VOIP);
                sendVideoBroadcast(intent);
                isConnected = false;

            }else if(mState == 3) {//视频接通之后发广播，小微将不能被唤醒
                QLog.i(TAG,"send voip enter broadcast,action = ACTION_ENTER_VOIP_CALL");
                Intent intent = new Intent();
                intent.setAction(ACTION_ENTER_VOIP);
                sendVideoBroadcast(intent);
                isConnected = true;
            }
            ...
            switch (mState) {
                case 0:
                    ...
                    mCurrentPeerId = 0;
                    //通话结束，关闭视频焦点
                    audioFocusManager.abandonAudioFocusForCall(mContext);
                    break;
                case 1:
    ...
    public void closeAudioChat(boolean isAudioOp) {
        QLog.d(TAG,"closeAudioChat isAudioOp ="+isAudioOp);
        if (mIXWAudioChatService != null) {
            try {
    ...
    public void acceptAudioChat(boolean isAudioOp) {
        QLog.d(TAG,"acceptAudioChat isAudioOp ="+isAudioOp);
        if (mIXWAudioChatService != null) {
            try {
    ...
    public void rejectAudioChat(boolean isAudioOp) {
        QLog.d(TAG,"rejectAudioChat isAudioOp ="+isAudioOp);
        if (mIXWAudioChatService != null) {
            try {
    ...
    public void startQQCallSkill(boolean invited, long uin) {
        ...
        recordService.init(mIsInvited, String.valueOf(mCurrentPeerId), String.valueOf(selfDin));
        recordService.initCreate();
        //获取audioFocus后即可停止其他占用的audioFocus，立即释放防止占用导致TTS无法播放
        audioFocusManager.requestAudioFocusForCall(mContext,AudioManager.STREAM_MUSIC,AudioManager.AUDIOFOCUS_GAIN);
        audioFocusManager.abandonAudioFocusForCall(mContext);
        QLog.d(TAG, "startQQCallSkill " + invited + " " + uin);
        if (mCurrentPeerId > 0 && invited && !privacyManager.isPrivacy())
            XWSDK.getInstance().requestProtocolTTS(mCurrentPeerId, 0, XWCommonDef.RequestProtocalType.CHAT, null);
        else {
            Log.e(TAG, "QQCall peerid is invalid.");
        }
    ...
    private BroadcastReceiver skillReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            if(intent.getAction().equals(ACTION_MAKE_VIDOE_CALL)){
    ...
    private BroadcastReceiver mScoBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            QLog.d(TAG,"mScoBroadcastReceiver sco_audio_state="+state);
